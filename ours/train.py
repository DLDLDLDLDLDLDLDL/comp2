...
def val_step(imgs, focal_loss, smooth_l1_loss, targets0, targets1, net, optimizer):
    regression, classification = net(imgs) # 用 efficientnet 做 classify
    reg_value = smooth_l1_loss(targets0, regression)
    cls_value = focal_loss(targets1, classification)
    loss_value = reg_value + cls_value # calculate loss
...
def fit_one_epoch(net, focal_loss, smooth_l1_loss, optimizer, epoch, epoch_size, epoch_size_val, gen, genval, Epoch, train_step=None):
    total_r_loss = 0
    total_c_loss = 0
    total_loss = 0
    val_loss = 0
